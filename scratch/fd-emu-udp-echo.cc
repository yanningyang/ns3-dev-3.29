...
main (int argc, char *argv[])
{
  ...
  Ipv4Address localIp ("192.168.79.100");
  ...
  NS_LOG_UNCOND("clientMode: " << clientMode);
  NS_LOG_UNCOND("serverMode: " << serverMode);
  ...
  if (serverMode)
    {
      ...
      UdpEchoServerHelper server (9999);
      apps = server.Install (n.Get (0));
      apps.Start (Seconds (1.0));
      apps.Stop (Seconds (stopTime));

      Config::Connect ("/NodeList/0/ApplicationList/0/$ns3::UdpEchoServer/RxWithAddresses", MakeCallback (&TwoAddressTrace));
    }
  else if (clientMode)
    {
      //
      ...
      UdpEchoClientHelper client (Ipv4Address ("192.168.79.1"), 9999);
