...
public:
   ...
   * \param routingTableFile output file for printing routing table
   * \return none
   */
...
private:
  ...
  double m_dataStartTime;	///start time of data transmission
  double m_dataEndTime;		///end time of data transmission
			...
			std::string routingTableFile)
{
  ...
  m_dataEndTime = dataEndTime;
  m_protocol = protocol;
  m_nSinks = nSinks;
  m_routingTables = routingTables;
  m_routingTableFile = routingTableFile;
...
RoutingHelper::SetupRoutingProtocol (NodeContainer & c)
{
  ...
  Ptr<OutputStreamWrapper> rtw = ascii.CreateFileStream (m_routingTableFile);
  ...
  switch (m_protocol)
    ...
    case 2:
      if (m_routingTables != 0)
        {
          aodv.PrintRoutingTableAllAt (Seconds (m_TotalSimTime), rtw);
        }
      list.Add (aodv, 100);
      m_protocolName = "AODV";
      break;
...
RoutingHelper::SetupRoutingMessages (NodeContainer & c,
                                     Ipv4InterfaceContainer & adhocTxInterfaces)
{
  NS_LOG_UNCOND ("Setup Routing Messages");
  ...
  for (uint32_t i = 0; i < m_nSinks; i++)
    {
      ...
      ApplicationContainer temp = onoff1.Install (c.Get (nodeNum - 1 - i));
      temp.Start (Seconds (var->GetValue (m_dataStartTime, m_dataStartTime + 1.0)));
      temp.Stop (Seconds (m_dataEndTime));
    }
}
...
VanetRoutingExperiment::VanetRoutingExperiment ()
  : m_workspacePath ("/home/haha/ns3.29-dev-workspace/workspace/vanet-routing-compare"),
    m_port (9),
    m_CSVfileName (),
    m_CSVfileName2 (),
    m_nSinks (10),
...
VanetRoutingExperiment::ProcessOutputs ()
{
  ...
  uint32_t packetsSend = m_routingHelper->GetRoutingStats ().GetTxPkts ();
  averageRoutingGoodputKbps = (((double) totalBytesTotal * 8.0) / m_TotalSimTime) / 1000.0;
  double aodv_pdr = (double)packetsReceived / packetsSend;

  if (m_log != 0)
    {
      NS_LOG_UNCOND (" PacketsReceived=" << packetsReceived <<
		     " packetsSend=" << packetsSend <<
		     " NumberOfSinks=" << m_nSinks <<
		     " RoutingProtocol=" << m_protocolName <<
		     " TransmissionPower=" << m_txp <<
		     " AverageRoutingGoodputKbps=" << averageRoutingGoodputKbps << "Kbps" <<
		     " AODV_PDR=" << aodv_pdr);
    }
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
  ...
  else if (m_scenario == 2)
    {
      ...
      m_nNodes = 80;
      m_nSinks = m_nNodes / 3;
      m_TotalSimTime = 200.01;
      m_dataStartTime = 160.01;
      m_dataEndTime = 180;
      m_mobility = 1;
      m_80211mode = 1;
      m_rate = "2048bps";
      std::string partOfFileName = "/highway.mobility-" + std::to_string(m_nNodes) + "nn-25-25-25-25-25s";
      m_traceFile = m_workspacePath + "/mobility" + partOfFileName + ".tcl";
      m_logFile = m_workspacePath + partOfFileName + ".log";
      m_CSVfileName = m_workspacePath + partOfFileName + ".csv";
      m_CSVfileName2 = m_workspacePath +  partOfFileName + "-2" + ".csv";
      m_routingTableFile = m_workspacePath + partOfFileName + "-routing-table.txt";
      m_loadConfigFilename = m_workspacePath + partOfFileName + "-load-config.txt";
    }
}
